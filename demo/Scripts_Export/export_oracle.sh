...
nohup expdp $DBUSERNAME/$DBPASSWORD@$ORACLE_SID schemas=$OBJET_NAME DIRECTORY=exp_dir DUMPFILE=expdpSHEMA_`echo $ORACLE_SID`_`echo $DATEFORMAT`.dmp LOGFILE=$EXPLOG > ${TFILE} 2>&1 &
...
tar -cvf $DUMPFILE.tar $DUMPFILE.dmp $EXPLOG
gzip $DUMPFILE.tar
